...
import {
  PlaneTakeoff, Plus, Check, X, Wallet, Clock, Ban,
  ChevronDown, Activity, BookOpen, Stethoscope, HelpCircle
} from 'lucide-react';
...
const statusConfig = {
  ...
  REJECTED:  { label: 'Rejected',  bg: 'bg-red-100',    text: 'text-red-800',    icon: X      },
  CANCELLED: { label: 'Cancelled', bg: 'bg-neutral-100',text: 'text-neutral-500',icon: Ban    },
};

const leaveTypeIcons = { Casual: Activity, Medical: Stethoscope, Legacy: BookOpen };
...
// ─── Balance Card ────────────────────────────────────────────────────────────
const BalanceCard = ({ type_name, available_balance, is_paid }) => {
  const balance = parseFloat(available_balance);
  const color = balance <= 0 ? 'text-red-500' : balance <= 2 ? 'text-amber-500' : 'text-emerald-500';
  return (
    <motion.div
    ...
    >
      ...
      {!is_paid && (
        <span className="text-[10px] px-1.5 py-0.5 bg-red-50 text-red-500 rounded-full mt-1">Unpaid</span>
      )}
    </motion.div>
  );
};
...
const RequestCard = ({ req, isAdmin, onStatusChange }) => {
  ...
  const Icon = cfg.icon;
  const daysText = parseFloat(req.days_consumed) === 0.5 ? '½ day' : `${parseFloat(req.days_consumed)} day(s)`;
      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-4">
        <div className="flex-1 space-y-2">
          ...
          <p className="text-sm text-neutral-500">
            <span className="font-medium text-neutral-700">{new Date(req.start_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
            {' → '}
            <span className="font-medium text-neutral-700">{new Date(req.end_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
          </p>
          ...
          {req.reviewer && (
            <p className="text-xs text-neutral-400">
              Reviewed by <span className="font-medium text-neutral-600">{req.reviewer.name}</span>
            </p>
          )}
        </div>
...
export const LeaveRequestHub = ({ user, isAdmin = false }) => {
  ...
  useEffect(() => {
    api.getLeaveTypes()
      .then(r => {
        ...
        if (active.length > 0) setForm(f => ({ ...f, leave_type_id: String(active[0].id) }));
      })
      .catch(console.error);
  }, []);
  ...
  const fetchBalance = useCallback(async () => {
    if (!isAdmin && user?.user_id) {
      try {
        ...
        setBalances(res.data.filter(b => b.leave_type_name !== 'Legacy'));
  ...
  const handleSubmit = async (e) => {
    ...
    const selectedBalance = balances.find(b => String(b.leave_type_id) === String(form.leave_type_id));
    const available = selectedBalance ? parseFloat(selectedBalance.available_balance) : null;
    ...
    if (available !== null && available < diffDays) {
      setError(`Insufficient balance. You have ${available.toFixed(1)} day(s) available but requested ${diffDays}.`);
      setSubmitting(false);
      return;
    }
    ...
    try {
      ...
      setShowModal(false);
      setForm(f => ({ ...f, start_date: '', end_date: '', reason: '', is_half_day: false }));
  ...
  const selectedTypeBalance = balances.find(b => String(b.leave_type_id) === String(form.leave_type_id));
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          ...
          <p className="text-neutral-500 mt-1 text-sm">
            {isAdmin ? 'Review and manage employee leave requests.' : 'Request leave and track your application status.'}
          </p>
        </div>
      ...
      <div className="space-y-3">
        {loading ? (
          <div className="flex items-center justify-center h-32 text-neutral-400 text-sm">Loading requests…</div>
        ) : requests.length === 0 ? (
          ...
          >
            ...
            >
              ...
              <div className="px-6 py-4 border-b border-neutral-100 flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-neutral-400">Submit a new leave application</p>
                  </div>
                </div>
                <button
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-5">
                ...
                {/* Leave Type */}
                <div>
                  <label className="block text-sm font-medium text-neutral-700 mb-1.5">Leave Type</label>
                  <div className="relative">
                    <select
                  ...
                  {selectedTypeBalance && (
                    <p className="mt-1.5 text-xs text-neutral-500 flex items-center gap-1">
                      <Wallet size={11} />
                      Available: <span className={`font-semibold ml-0.5 ${parseFloat(selectedTypeBalance.available_balance) <= 0 ? 'text-red-500' : 'text-emerald-600'}`}>
                        {parseFloat(selectedTypeBalance.available_balance).toFixed(1)} days
                      </span>
                    </p>
                  )}
                </div>
                ...
                <div>
                  <label className="flex items-center gap-3 cursor-pointer group">
                    ...
                    >
                      <span className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform duration-200 ${form.is_half_day ? 'translate-x-5' : ''}`} />
                    </div>
                        ...
                        <div className="flex gap-2">
                          {['morning', 'afternoon'].map(session => (
                            <button
                              key={session}
                              type="button"
                              onClick={() => setForm(f => ({ ...f, half_day_session: session }))}
